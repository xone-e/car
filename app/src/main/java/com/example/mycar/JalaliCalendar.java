...
public class JalaliCalendar extends Calendar {
        ...
        if (field == MONTH) {
            if (amount>=0) {
                ...
                if (amount<0) {
                    ...
                    super.set(MONTH, (amount % 12)+12);
                    if (get(DAY_OF_MONTH) > jalaliDaysInMonth[(amount % 12)+12]) {
                        super.set(DAY_OF_MONTH, jalaliDaysInMonth[(amount % 12)+12]);
                        if (get(MONTH) == 11 && isLeepYear(get(YEAR))) {
                            super.set(DAY_OF_MONTH, 30);
                        }
                    }
    ...
    public static String monthName(int month) {
        final String[] persianMonthName = { "فروردین",
    ...
    public static String weekName(int week) {
        final String[] weekdayName = {"", "یکشنبه", "دوشنبه",
    ...
    public static JalaliCalendar GregorianToJalali(int year , int month , int day , int hour , int minute , int second) {
        YearMonthDate yearMonthDate = new YearMonthDate(year,month-1,day);
        YearMonthDate jalaliCalendar = JalaliCalendar.gregorianToJalali(yearMonthDate);
        return new JalaliCalendar(jalaliCalendar.getYear(),jalaliCalendar.getMonth(),jalaliCalendar.getDate() , hour , minute , second);
        //return jdnToPersian(civilToJdn(civil));
    }


    public static Calendar JalaliToGregorian(int year , int month , int day , int hour , int minute , int second) {
        YearMonthDate yearMonthDate = new YearMonthDate(year,month-1,day);
        YearMonthDate jalaliCalendar = JalaliCalendar.jalaliToGregorian(yearMonthDate);
        Calendar calendar = Calendar.getInstance();
        calendar.set(jalaliCalendar.getYear() , jalaliCalendar.getMonth() , jalaliCalendar.getDate() , hour , minute , second);
